...
public class Class196 {
	...
	public static int[][] anIntArrayArray2411 = new int[2][8];
	static {
	...
	private static final float method1996(float f) {
		float f_17_ = 32.703197F * (float) Math.pow(2.0, f);
	...
	private final float method1991(int i, int i_0_, float f) {
		float f_1_ = anIntArrayArrayArray2408[i][0][i_0_]
				+ f * (anIntArrayArrayArray2408[i][1][i_0_] - anIntArrayArrayArray2408[i][0][i_0_]);
		f_1_ *= 0.0015258789F;
		return 1.0F - (float) Math.pow(10.0, -f_1_ / 20.0F);
	}

	public final int method1993(int i, float f) {
		if (i == 0) {
			float f_2_ = anIntArray2405[0] + (anIntArray2405[1] - anIntArray2405[0]) * f;
			f_2_ *= 0.0030517578F;
			aFloat2404 = (float) Math.pow(0.1, f_2_ / 20.0F);
			anInt2406 = (int) (aFloat2404 * 65536.0F);
		}
		if (anIntArray2409[i] == 0)
			return 0;
		float f_3_ = method1991(i, 0, f);
		aFloatArrayArray2407[i][0] = -2.0F * f_3_ * (float) Math.cos(method1995(i, 0, f));
		aFloatArrayArray2407[i][1] = f_3_ * f_3_;
		for (int i_4_ = 1; i_4_ < anIntArray2409[i]; i_4_++) {
			f_3_ = method1991(i, i_4_, f);
			float f_5_ = -2.0F * f_3_ * (float) Math.cos(method1995(i, i_4_, f));
			float f_6_ = f_3_ * f_3_;
			aFloatArrayArray2407[i][i_4_ * 2 + 1] = aFloatArrayArray2407[i][i_4_ * 2 - 1] * f_6_;
			aFloatArrayArray2407[i][i_4_ * 2] = aFloatArrayArray2407[i][i_4_ * 2 - 1] * f_5_
					+ aFloatArrayArray2407[i][i_4_ * 2 - 2] * f_6_;
			for (int i_7_ = i_4_ * 2 - 1; i_7_ >= 2; i_7_--)
				aFloatArrayArray2407[i][i_7_] += aFloatArrayArray2407[i][i_7_ - 1] * f_5_
						+ aFloatArrayArray2407[i][i_7_ - 2] * f_6_;
	...
	public final void method1994(BufferedStream buffer, Class177 class177) {
	...
	private final float method1995(int i, int i_15_, float f) {
		float f_16_ = anIntArrayArrayArray2410[i][0][i_15_]
				+ f * (anIntArrayArrayArray2410[i][1][i_15_] - anIntArrayArrayArray2410[i][0][i_15_]);
		f_16_ *= 1.2207031E-4F;
