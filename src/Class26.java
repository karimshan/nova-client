


/* Class26 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */

public class Class26 {
	public static IncomingPacket aClass192_455 = new IncomingPacket(96, 7);
	public static int anInt453;
	public static int anInt454;
	public static int anInt456;
	public static int anInt457;

	public static final boolean method311(int i, int i_0_) {
		if (i_0_ != -5)
			method312('\uffdc', (byte) 89);
		anInt454++;
		if ((i ^ 0xffffffff) != -4 && (i ^ 0xffffffff) != -5 && (i ^ 0xffffffff) != -6 && i != 6)
			return false;
		return true;
	}

	public static final byte method312(char c, byte b) {
		anInt456++;
		byte b_1_;
		if (((c ^ 0xffffffff) >= -1 || (c ^ 0xffffffff) <= -129)
				&& ((c ^ 0xffffffff) > -161 || (c ^ 0xffffffff) < -256)) {
			if (c == 8364)
				b_1_ = (byte) -128;
			else if ((c ^ 0xffffffff) != -8219) {
				if ((c ^ 0xffffffff) == -403)
					b_1_ = (byte) -125;
				else if (c != 8222) {
					if (c != 8230) {
						if ((c ^ 0xffffffff) == -8225)
							b_1_ = (byte) -122;
						else if ((c ^ 0xffffffff) == -8226)
							b_1_ = (byte) -121;
						else if ((c ^ 0xffffffff) != -711) {
							if ((c ^ 0xffffffff) != -8241) {
								if (c == 352)
									b_1_ = (byte) -118;
								else if (c != 8249) {
									if ((c ^ 0xffffffff) == -339)
										b_1_ = (byte) -116;
									else if ((c ^ 0xffffffff) != -382) {
										if ((c ^ 0xffffffff) == -8217)
											b_1_ = (byte) -111;
										else if (c != 8217) {
											if ((c ^ 0xffffffff) != -8221) {
												if (c == 8221)
													b_1_ = (byte) -108;
												else if (c == 8226)
													b_1_ = (byte) -107;
												else if (c != 8211) {
													if ((c ^ 0xffffffff) == -8213)
														b_1_ = (byte) -105;
													else if (c == 732)
														b_1_ = (byte) -104;
													else if (c == 8482)
														b_1_ = (byte) -103;
													else if ((c ^ 0xffffffff) != -354) {
														if ((c ^ 0xffffffff) == -8251)
															b_1_ = (byte) -101;
														else if (c != 339) {
															if ((c ^ 0xffffffff) == -383)
																b_1_ = (byte) -98;
															else if ((c ^ 0xffffffff) == -377)
																b_1_ = (byte) -97;
															else
																b_1_ = (byte) 63;
														} else
															b_1_ = (byte) -100;
													} else
														b_1_ = (byte) -102;
												} else
													b_1_ = (byte) -106;
											} else
												b_1_ = (byte) -109;
										} else
											b_1_ = (byte) -110;
									} else
										b_1_ = (byte) -114;
								} else
									b_1_ = (byte) -117;
							} else
								b_1_ = (byte) -119;
						} else
							b_1_ = (byte) -120;
					} else
						b_1_ = (byte) -123;
				} else
					b_1_ = (byte) -124;
			} else
				b_1_ = (byte) -126;
		} else
			b_1_ = (byte) c;
		if (b >= -26)
			aClass192_455 = null;
		return b_1_;
	}

	public static final void method313(byte b, int i) {
		Class320_Sub16.anInt8360 = i;
		anInt457++;
		Class105.aClass61_5202.method608(false);
		@SuppressWarnings("unused")
		int i_2_ = -1 / ((b - -24) / 57);
	}

	public static void method314(byte b) {
		aClass192_455 = null;
		if (b != 100)
			aClass192_455 = null;
	}

	@Override
	public final String toString() {
		anInt453++;
		throw new IllegalStateException();
	}
}
